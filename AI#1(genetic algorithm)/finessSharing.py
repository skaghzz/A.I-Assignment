...
def distanceTwoPoint(x1,x2,y1,y2):
    return sqrt((x1-x2)**2 + (y1-y2)**2)

def sumOfDistance(individual):
    global everyPop
    sumDistance = 0.0
    #sigma = 30*sqrt(2)
    sigma = 60*sqrt(2)
    #sigma = 1000.0
    for i in range(len(everyPop)):
        x1 = chromosomeDesign(individual[0:8])
        y1 = chromosomeDesign(individual[8:16])
        x2 = chromosomeDesign((everyPop[i])[0:8])
        y2 = chromosomeDesign((everyPop[i])[8:16])
        d = distanceTwoPoint(x1,x2,y1,y2)
        if(d < sigma):
            sumDistance += 1 - (d/sigma)
    return sumDistance
...
def showFitnessGraph(nthGeneration, avgList):
    ...
    plt1.axis([-1, nthGeneration, -0.001, 0.005])
...
def fitnessSharing(individual):
    return GrieWank(individual) / sumOfDistance(individual),

# ----------
# Operator registration
# ----------
# register the goal / fitness function
toolbox.register("evaluate", fitnessSharing)
